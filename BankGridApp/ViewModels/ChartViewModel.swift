...
class ChartViewModel: ObservableObject {
    ...
    private func analyzeMACD(dif: Double, macd: Double, macdIncreasing: Bool) -> (text: String, colorHex: String, detail: String) {
        if dif > 0 {
            if macd > 0 {
                if macdIncreasing {
                    return ("强多�?加�?", "#ff3b30", "当前DIF在零轴上方且MACD柱为正并在增大，多头趋势强劲，上涨动能持续加速。此阶段持股为主，但需警惕短期过热�?)
                } else {
            ...
            if macd < 0 {
                if macdIncreasing {
                    ...
                    return ("强空�?加�?", "#34C759", "当前DIF在零轴下方且MACD柱为负并在扩大，空头趋势强劲，下跌动能持续加速。此阶段应以观望为主，不宜盲目抄底�?)
                }
            } else {
                if macdIncreasing {
    ...
    private func analyzeRSI(rsi6: Double, rsi14: Double) -> (text: String, colorHex: String, detail: String) {
        if rsi6 > 80 {
            ...
            return ("超买�?, "#FF9F0A", "RSI(6)超过70，进入超买区域。短期上涨较快，市场情绪偏热，存在回调压力。建议谨慎追涨，可考虑部分获利了结，等待RSI回落后再考虑介入�?)
        } else if rsi6 >= 50 && rsi6 <= 70 && rsi6 > rsi14 {
            return ("多头偏强", "#FF6B8A", "RSI(6)�?0-70之间且高于RSI(14)，多头力量占优。短期动能偏强，价格有继续上行的可能。可维持当前仓位，但需关注RSI是否逼近70超买线�?)
        } else if rsi6 >= 40 && rsi6 <= 60 {
            return ("中性区�?, "#8e8e93", "RSI(6)�?0-60之间，多空力量相对均衡，市场处于震荡整理阶段。此区间方向不明，建议观望为主，等待RSI突破50后确认方向再行动�?)
        } else if rsi6 >= 30 && rsi6 < 50 && rsi6 < rsi14 {
            return ("空头偏强", "#64D2FF", "RSI(6)�?0-50之间且低于RSI(14)，空头力量占优。短期动能偏弱，价格有继续下行的可能。建议谨慎操作，等待RSI回升突破50后再考虑加仓�?)
        } else if rsi6 >= 20 && rsi6 < 30 {
            return ("超卖�?, "#BF5AF2", "RSI(6)低于30，进入超卖区域。短期跌幅较大，市场情绪偏悲观，但存在技术性反弹的可能。可关注是否出现企稳信号，不宜盲目杀跌�?)
        } else if rsi6 < 20 {
