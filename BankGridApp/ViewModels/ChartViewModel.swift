...
class ChartViewModel: ObservableObject {
    ...
    private func analyzeMACD(dif: Double, macd: Double, macdIncreasing: Bool) -> (text: String, colorHex: String, detail: String) {
        if dif > 0 {
            if macd > 0 {
                if macdIncreasing {
                    return ("强多头加速", "#ff3b30", "当前DIF在零轴上方且MACD柱为正并在增大，多头趋势强劲，上涨动能持续加速。此阶段持股为主，但需警惕短期过热。")
                } else {
            ...
            if macd < 0 {
                if macdIncreasing {
                    ...
                    return ("强空头加速", "#34C759", "当前DIF在零轴下方且MACD柱为负并在扩大，空头趋势强劲，下跌动能持续加速。此阶段应以观望为主，不宜盲目抄底。")
                }
            } else {
                if macdIncreasing {
